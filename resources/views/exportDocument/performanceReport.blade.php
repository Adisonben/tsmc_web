                        ...
                        <table class="table table-bordered border-secondary">
                            <thead class="text-center">
                                <tr>
                                    ...
                                    <th scope="col" style="background-color: #E7D3EF">จำนวนที่ออกรายงาน (ครั้ง)</th>
                                    <th scope="col" style="background-color: #E7D3EF">จำนวนรถ (คัน)</th>
                                    ...
                                    @foreach ($categories as $index => $cate)
                                        @if (count($cate->getForms ?? []) > 0)
                                            ...
                                            @foreach ($cate->getForms ?? [] as $index => $form)
                                                <tr>
                                                    <td>{{ $index + 1 }} {{ $form->title }}</td>
                                                    <td class="text-center">{{ $form->countExportByQuarter($quarter) }}</td>
                                                    <td class="text-center">{{ $form->countVehicleFromSubmissionByQuarter($quarter) }}</td>
